# This file was generated by Scalar::Random::OO::Maker 0.06 from Mouse 0.65.
#
# ANY CHANGES MADE HERE WILL BE LOST!
use strict;
use warnings;

# tell Perl we already have all of the Scalar::Random::OO files loaded:
BEGIN {
	$INC{'Scalar/Random/OO.pm'}                             = __FILE__;
	$INC{'Scalar/Random/OO/Exporter.pm'}                    = __FILE__;
	$INC{'Scalar/Random/OO/Object.pm'}                      = __FILE__;
	$INC{'Scalar/Random/OO/PurePerl.pm'}                    = __FILE__;
	$INC{'Scalar/Random/OO/Role.pm'}                        = __FILE__;
	$INC{'Scalar/Random/OO/Util.pm'}                        = __FILE__;
	$INC{'Scalar/Random/OO/Meta/Attribute.pm'}              = __FILE__;
	$INC{'Scalar/Random/OO/Meta/Class.pm'}                  = __FILE__;
	$INC{'Scalar/Random/OO/Meta/Method.pm'}                 = __FILE__;
	$INC{'Scalar/Random/OO/Meta/Module.pm'}                 = __FILE__;
	$INC{'Scalar/Random/OO/Meta/Role.pm'}                   = __FILE__;
	$INC{'Scalar/Random/OO/Meta/TypeConstraint.pm'}         = __FILE__;
	$INC{'Scalar/Random/OO/Meta/Method/Accessor.pm'}        = __FILE__;
	$INC{'Scalar/Random/OO/Meta/Method/Constructor.pm'}     = __FILE__;
	$INC{'Scalar/Random/OO/Meta/Method/Delegation.pm'}      = __FILE__;
	$INC{'Scalar/Random/OO/Meta/Method/Destructor.pm'}      = __FILE__;
	$INC{'Scalar/Random/OO/Meta/Role/Composite.pm'}         = __FILE__;
	$INC{'Scalar/Random/OO/Meta/Role/Method.pm'}            = __FILE__;
	$INC{'Scalar/Random/OO/Util/MetaRole.pm'}               = __FILE__;
	$INC{'Scalar/Random/OO/Util/TypeConstraints.pm'}        = __FILE__;
}

# and now their contents

# Contents of Mouse::PurePerl
package Scalar::Random::OO::PurePerl;

require Scalar::Random::OO::Util;

package Scalar::Random::OO::Util;

use strict;
use warnings;

use warnings FATAL => 'redefine'; # to avoid to load Scalar::Random::OO::PurePerl
...
sub generate_isa_predicate_for {
	...
	if(defined $name){
		Scalar::Random::OO::Util::install_subroutines(scalar caller, $name => $predicate);
		return;
	}
...
package Scalar::Random::OO::Util::TypeConstraints;
...
sub ClassName  { Scalar::Random::OO::Util::is_class_loaded($_[0]) }
sub RoleName   { (Scalar::Random::OO::Util::class_of($_[0]) || return 0)->isa('Scalar::Random::OO::Meta::Role') }
...
package Scalar::Random::OO::Meta::Module;
...
sub add_method {
	...
	Scalar::Random::OO::Util::install_subroutines($self->name,
		$name => $code,
	);
	return;
}

package Scalar::Random::OO::Meta::Class;

use Scalar::Random::OO::Meta::Method::Constructor;
use Scalar::Random::OO::Meta::Method::Destructor;

sub method_metaclass    { $_[0]->{method_metaclass}    || 'Scalar::Random::OO::Meta::Method'    }
sub attribute_metaclass { $_[0]->{attribute_metaclass} || 'Scalar::Random::OO::Meta::Attribute' }

sub constructor_class { $_[0]->{constructor_class} || 'Scalar::Random::OO::Meta::Method::Constructor' }
sub destructor_class  { $_[0]->{destructor_class}  || 'Scalar::Random::OO::Meta::Method::Destructor'  }
...
sub new_object {
	...
	if( $object->can('BUILD') ) {
		for my $class (reverse $meta->linearized_isa) {
			my $build = Scalar::Random::OO::Util::get_code_ref($class, 'BUILD')
				|| next;
...
sub clone_object {
	my $class  = shift;
	my $object = shift;
	my $args   = $object->Scalar::Random::OO::Object::BUILDARGS(@_);
...
sub strict_constructor{
	...
	foreach my $class($self->linearized_isa) {
		my $meta = Scalar::Random::OO::Util::get_metaclass_by_name($class)
			or next;
...
sub _report_unknown_args {
	...
	$metaclass->throw_error( sprintf
		...
		$metaclass->name, Scalar::Random::OO::Util::english_list(@unknowns),
	);
}

package Scalar::Random::OO::Meta::Role;

sub method_metaclass{ $_[0]->{method_metaclass} || 'Scalar::Random::OO::Meta::Role::Method' }
...
package Scalar::Random::OO::Meta::Attribute;

require Scalar::Random::OO::Meta::Method::Accessor;

sub accessor_metaclass{ $_[0]->{accessor_metaclass} || 'Scalar::Random::OO::Meta::Method::Accessor' }
...
sub _process_options{
	...
	if(exists $args->{builder}){
		# XXX:
		# Moose refuses a CODE ref builder, but Scalar::Random::OO doesn't for backward compatibility
	...
	# taken from Scalar::Random::OO::Meta::Attribute->new and ->_process_args
	...
	my $tc;
	if(exists $args->{isa}){
		$tc = $args->{type_constraint} = Scalar::Random::OO::Util::TypeConstraints::find_or_create_isa_type_constraint($args->{isa});
	}
	...
	if(exists $args->{does}){
		if(defined $tc){ # both isa and does supplied
			...
			$tc = $args->{type_constraint} = Scalar::Random::OO::Util::TypeConstraints::find_or_create_does_type_constraint($args->{does});
		}
	}
...
package Scalar::Random::OO::Meta::TypeConstraint;
...
sub compile_type_constraint{
	...
	if(@checks == 0){
		$self->{compiled_type_constraint} = \&Scalar::Random::OO::Util::TypeConstraints::Any;
	}
	else{
...
package Scalar::Random::OO::Object;
...
sub new {
	...
	my $meta = Scalar::Random::OO::Meta::Class->initialize($class);
...
sub DESTROY {
	...
	my $e = do{
		...
		eval{
			...
			foreach my $class (@{ Scalar::Random::OO::Util::get_linear_isa(ref $self) }) {
				my $demolish = Scalar::Random::OO::Util::get_code_ref($class, 'DEMOLISH')
					|| next;

				$self->$demolish($Scalar::Random::OO::Util::in_global_destruction);
			}
		};
		$@;
	};
...
package Scalar::Random::OO::Exporter;
use strict;
use warnings;
...
# it must be "require", because Scalar::Random::OO::Util depends on Scalar::Random::OO::Exporter,
# which depends on Scalar::Random::OO::Util::import()
require Scalar::Random::OO::Util;
...
sub setup_import_methods{
	...
	Scalar::Random::OO::Util::install_subroutines($exporting_package,
		import   => $import,
		unimport => $unimport,
	...
	foreach my $package(@export_from){
		...
		if(my $as_is = $spec->{as_is}){
			foreach my $thingy (@{$as_is}){
				...
				if(ref($thingy)){
					$code = $thingy;
					($code_package, $code_name) = Scalar::Random::OO::Util::get_code_info($code);
				}
				else{
...
sub do_import {
	...
	my $spec = $SPEC{$package}
		|| confess("The package $package package does not use Scalar::Random::OO::Exporter");
	...
	while(@args){
		...
		if($arg =~ s/^-//){
			if($arg eq 'traits'){
				...
				Scalar::Random::OO::Util::not_supported("-$arg");
			}
		}
		elsif($arg =~ s/^://){
	...
	if($spec->{INIT_META}){
		...
		if(@traits){
			...
			@traits =
				map{
					ref($_) ? $_
							: Scalar::Random::OO::Util::resolve_metaclass_alias($type => $_, trait => 1)
				} @traits;

			require Scalar::Random::OO::Util::MetaRole;
			Scalar::Random::OO::Util::MetaRole::apply_metaroles(
				for       => $into,
				Scalar::Random::OO::Util::is_a_metarole($into->meta)
	...
	if(@exports){
		...
		Scalar::Random::OO::Util::install_subroutines($into, @export_table);
	}
	else{
		Scalar::Random::OO::Util::install_subroutines($into, %{$spec->{DEFAULT}});
	}
	return;
}
...
sub do_unimport {
	...
	my $spec = $SPEC{$package}
		|| confess("The package $package does not use Scalar::Random::OO::Exporter");
...
package Scalar::Random::OO::Util;
use Scalar::Random::OO::Exporter; # enables strict and warnings
no warnings 'once';

# must be here because it will be refered by other modules loaded
sub get_linear_isa($;$); ## no critic

# must be here because it will called in Scalar::Random::OO::Exporter
...
BEGIN{
	# This is used in Scalar::Random::OO::PurePerl
	...
	# Because Scalar::Random::OO::Util is loaded first in all the Scalar::Random::OO sub-modules,
	# XS loader is placed here, not in Scalar/Random/OO.pm.

	our $VERSION = '0.65';

	my $xs = !(exists $INC{'Scalar/Random/OO/PurePerl.pm'} || $ENV{MOUSE_PUREPERL} || $ENV{PERL_ONLY});

	if($xs){
		# XXX: XSLoader tries to get the object path from caller's file name
		#      $hack_mouse_file fools its mechanism

		(my $hack_mouse_file = __FILE__) =~ s/.Util//; # .../Scalar/Random/OO/Util.pm -> .../Scalar/Random/OO.pm
		$xs = eval sprintf("#line %d %s\n", __LINE__, $hack_mouse_file) . q{
			local $^W = 0; # work around 'redefine' warning to &install_subroutines
			require XSLoader;
			XSLoader::load('Scalar::Random::OO', $VERSION);
			Scalar::Random::OO::Util->import({ into => 'Scalar::Random::OO::Meta::Method::Constructor::XS' }, ':meta');
			Scalar::Random::OO::Util->import({ into => 'Scalar::Random::OO::Meta::Method::Destructor::XS'  }, ':meta');
			Scalar::Random::OO::Util->import({ into => 'Scalar::Random::OO::Meta::Method::Accessor::XS'    }, ':meta');
			return 1;
		} || 0;
		#warn $@ if $@;
	}
	...
	if(!$xs){
		require 'Scalar/Random/OO/PurePerl.pm'; # we don't want to create its namespace
	}
...
# it must be 'require', not 'use', because Scalar::Random::OO::Meta::Module depends on Scalar::Random::OO::Util
require Scalar::Random::OO::Meta::Module; # for the entities of metaclass cache utilities

# aliases
{
	*class_of                    = \&Scalar::Random::OO::Meta::Module::_class_of;
	*get_metaclass_by_name       = \&Scalar::Random::OO::Meta::Module::_get_metaclass_by_name;
	*get_all_metaclass_instances = \&Scalar::Random::OO::Meta::Module::_get_all_metaclass_instances;
	*get_all_metaclass_names     = \&Scalar::Random::OO::Meta::Module::_get_all_metaclass_names;

	*Scalar::Random::OO::load_class           = \&load_class;
	*Scalar::Random::OO::is_class_loaded      = \&is_class_loaded;

	# is-a predicates
	#generate_isa_predicate_for('Scalar::Random::OO::Meta::TypeConstraint' => 'is_a_type_constraint');
	#generate_isa_predicate_for('Scalar::Random::OO::Meta::Class'          => 'is_a_metaclass');
	#generate_isa_predicate_for('Scalar::Random::OO::Meta::Role'           => 'is_a_metarole');
...
sub does_role{
	...
	(defined $role_name)
		|| ($meta || 'Scalar::Random::OO::Meta::Class')->throw_error("You must supply a role name to does()");
...
# taken from Scalar::Random::OO::Util (0.90)
{
	my %cache;
	...
	sub resolve_metaclass_alias {
		...
		return $cache{$cache_key}{$metaclass_name} ||= do{
			...
			my $possible_full_name = join '::',
				'Scalar::Random::OO::Meta', $type, 'Custom', ($options{trait} ? 'Trait' : ()), $metaclass_name
			;
...
sub apply_all_roles {
	my $consumer = Scalar::Util::blessed($_[0])
		?                                shift   # instance
		: Scalar::Random::OO::Meta::Class->initialize(shift); # class or role name
	...
	for (my $i = 0; $i < $max ; $i++) {
		...
		is_a_metarole( get_metaclass_by_name($role_name) )
			|| $consumer->meta->throw_error("You can only consume roles, $role_name is not a Scalar::Random::OO role");
	}
	...
	if ( scalar @roles == 1 ) {
		...
		Scalar::Random::OO::Meta::Role->combine(@roles)->apply($consumer);
	}
	return;
}
...
sub not_supported{
	...
	Carp::confess("Scalar::Random::OO does not currently support $feature");
}

# general meta() method
sub meta :method{
	return Scalar::Random::OO::Meta::Class->initialize(ref($_[0]) || $_[0]);
}
...
package Scalar::Random::OO::Meta::TypeConstraint;
use Scalar::Random::OO::Util qw(:meta); # enables strict and warnings
use Scalar::Util ();
...
use overload
	...
	'|'      => sub {                  # or-combination
		require Scalar::Random::OO::Util::TypeConstraints;
		return Scalar::Random::OO::Util::TypeConstraints::find_or_parse_type_constraint(
			"$_[0] | $_[1]",
		);
	},
...
sub _add_type_coercions{
	...
	for(my $i = 0; $i < @_; $i++){
		...
		my $type = Scalar::Random::OO::Util::TypeConstraints::find_or_parse_type_constraint($from)
...
sub parameterize{
	...
	if(!ref $param){
		require Scalar::Random::OO::Util::TypeConstraints;
		$param = Scalar::Random::OO::Util::TypeConstraints::find_or_create_isa_type_constraint($param);
	}
	...
	return Scalar::Random::OO::Meta::TypeConstraint->new(
		name           => $name,
		parent         => $self,
...
sub throw_error {
	require Scalar::Random::OO::Meta::Module;
	goto &Scalar::Random::OO::Meta::Module::throw_error;
}

# Contents of Mouse::Util::TypeConstraints
package Scalar::Random::OO::Util::TypeConstraints;
use Scalar::Random::OO::Util qw(does_role not_supported); # enables strict and warnings

use Carp         ();
use Scalar::Util ();

use Scalar::Random::OO::Meta::TypeConstraint;
use Scalar::Random::OO::Exporter;
...
our @CARP_NOT = qw(Scalar::Random::OO::Meta::Attribute);

my %TYPE;

# The root type
$TYPE{Any} = Scalar::Random::OO::Meta::TypeConstraint->new(
	name => 'Any',
);
...
while (my ($name, $parent, $code) = splice @builtins, 0, 3) {
	$TYPE{$name} = Scalar::Random::OO::Meta::TypeConstraint->new(
		name      => $name,
		parent    => $TYPE{$parent},
		optimized => $code,
	);
...
sub _create_type{
	...
	if(defined $name){
		...
		if(!$this){
			$this = caller(1);
			if($this !~ /\A Scalar::Random::OO \b/xms){
	...
	if($mode eq 'subtype'){
		...
		$constraint = Scalar::Random::OO::Meta::TypeConstraint->new(%args);
	}
...
sub class_type {
	...
	return _create_type 'subtype', $name => (
		as           => 'Object',
		optimized_as => Scalar::Random::OO::Util::generate_isa_predicate_for($class),
	);
}
...
sub duck_type {
	...
	return _create_type 'subtype', $name => (
		as           => 'Object',
		optimized_as => Scalar::Random::OO::Util::generate_can_predicate_for(\@methods),
	);
}
...
sub _find_or_create_regular_type{
	...
	my $meta = Scalar::Random::OO::Util::get_metaclass_by_name($spec);

	if(!defined $meta){
		return $create ? class_type($spec) : undef;
	}

	if(Scalar::Random::OO::Util::is_a_metarole($meta)){
...
sub find_type_constraint {
	my($spec) = @_;
	return $spec if Scalar::Random::OO::Util::is_a_type_constraint($spec);
...
sub register_type_constraint {
	...
	Carp::croak("No type supplied / type is not a valid type constraint")
		unless Scalar::Random::OO::Util::is_a_type_constraint($constraint);
...
sub find_or_create_does_type_constraint{
	# XXX: Moose does not register a new role_type, but Scalar::Random::OO does.
	return find_or_parse_type_constraint(@_) || role_type(@_);
}

sub find_or_create_isa_type_constraint {
	# XXX: Moose does not register a new class_type, but Scalar::Random::OO does.
	return find_or_parse_type_constraint(@_) || class_type(@_);
}

# Contents of Mouse
package Scalar::Random::OO::TOP;
use 5.006_002;
...
use Scalar::Random::OO::Util ();

use Scalar::Random::OO::Meta::Module;
use Scalar::Random::OO::Meta::Class;
use Scalar::Random::OO::Meta::Role;
use Scalar::Random::OO::Meta::Attribute;
use Scalar::Random::OO::Object;
use Scalar::Random::OO::Util::TypeConstraints ();
...
sub extends {
	Scalar::Random::OO::Meta::Class->initialize(scalar caller)->superclasses(@_);
	return;
}

sub with {
	Scalar::Random::OO::Util::apply_all_roles(scalar(caller), @_);
	return;
}

sub has {
	my $meta = Scalar::Random::OO::Meta::Class->initialize(scalar caller);
	my $name = shift;
...
sub override {
	# my($name, $method) = @_;
	Scalar::Random::OO::Meta::Class->initialize(scalar caller)->add_override_method_modifier(@_);
}
...
sub augment {
	#my($name, $method) = @_;
	Scalar::Random::OO::Meta::Class->initialize(scalar caller)->add_augment_method_modifier(@_);
	return;
}
...
sub init_meta {
	...
	my $base_class = $args{base_class} || 'Scalar::Random::OO::Object';
	my $metaclass  = $args{metaclass}  || 'Scalar::Random::OO::Meta::Class';
	...
	# make a class type for each Scalar::Random::OO class
	Scalar::Random::OO::Util::TypeConstraints::class_type($class)
		unless Scalar::Random::OO::Util::TypeConstraints::find_type_constraint($class);
...
my %valid_options = map { $_ => undef } (
  ...
  # Moose defines, but Scalar::Random::OO doesn't
  #'definition_context',
  #'initializer',
...
our @CARP_NOT = qw(Scalar::Random::OO::Meta::Class);

sub new {
	my $class = shift;
	my $name  = shift;

	my $args  = $class->Scalar::Random::OO::Object::BUILDARGS(@_);
	...
	if(@bad){
		Carp::carp(
			...
			. Scalar::Random::OO::Util::english_list(@bad));
	}
...
sub interpolate_class{
	...
	if(my $metaclass = delete $args->{metaclass}){
		$class = Scalar::Random::OO::Util::resolve_metaclass_alias( Attribute => $metaclass );
	}
	...
	if(my $traits_ref = delete $args->{traits}){
		...
		for (my $i = 0; $i < @{$traits_ref}; $i++) {
			my $trait = Scalar::Random::OO::Util::resolve_metaclass_alias(Attribute => $traits_ref->[$i], trait => 1);
		...
		if (@traits) {
			$class = Scalar::Random::OO::Meta::Class->create_anon_class(
				superclasses => [ $class ],
				roles        => \@traits,
				cache        => 1,
			)->name;
...
sub clone_and_inherit_options{
	my $self = shift;
	my $args = $self->Scalar::Random::OO::Object::BUILDARGS(@_);
...
sub delegation_metaclass() { ## no critic
	'Scalar::Random::OO::Meta::Method::Delegation'
}
...
sub _canonicalize_handles {
	...
	if (ref($handles) eq 'HASH') {
		...
		return $handles->( $self, Scalar::Random::OO::Meta::Class->initialize("$class_or_role"));
	}
	elsif (ref($handles) eq 'Regexp') {
		my $class_or_role = ($self->{isa} || $self->{does})
			|| $self->throw_error("Cannot delegate methods based on a Regexp without a type constraint (isa)");

		my $meta = Scalar::Random::OO::Meta::Class->initialize("$class_or_role"); # "" for stringify
		return map  { $_ => $_ }
			   grep { !Scalar::Random::OO::Object->can($_) && $_ =~ $handles }
				   Scalar::Random::OO::Util::is_a_metarole($meta)
...
sub _make_delegation_method {
	...
	return Scalar::Random::OO::Util::load_class($self->delegation_metaclass)
...
sub throw_error{
	...
	my $metaclass = (ref $self && $self->associated_class) || 'Scalar::Random::OO::Meta::Class';
	$metaclass->throw_error(@_, depth => 1);
}

# Contents of Mouse::Meta::Class
package Scalar::Random::OO::Meta::Class;
use Scalar::Random::OO::Util qw/:meta get_linear_isa not_supported/; # enables strict and warnings
no warnings 'once';

use Scalar::Util qw/blessed weaken/;

use Scalar::Random::OO::Meta::Module;
our @ISA = qw(Scalar::Random::OO::Meta::Module);

our @CARP_NOT = qw(Scalar::Random::OO); # trust Mousse
...
sub superclasses {
	...
	if (@_) {
		foreach my $super(@_){
			Scalar::Random::OO::Util::load_class($super);
			my $meta = Scalar::Random::OO::Util::get_metaclass_by_name($super);

			next if not defined $meta;

			if(Scalar::Random::OO::Util::is_a_metarole($meta)){
				$self->throw_error("You cannot inherit from a Scalar::Random::OO Role ($super)");
			}
...
my @MetaClassTypes = (
	'attribute',   # Scalar::Random::OO::Meta::Attribute
	'method',      # Scalar::Random::OO::Meta::Method
	'constructor', # Scalar::Random::OO::Meta::Method::Constructor
	'destructor',  # Scalar::Random::OO::Meta::Method::Destructor
);
...
sub _reconcile_with_superclass_meta {
	...
	require Scalar::Random::OO::Util::MetaRole;
	$_[0] = Scalar::Random::OO::Util::MetaRole::apply_metaroles(
		for             => $self,
...
sub get_all_method_names {
	...
	return grep { $uniq{$_}++ == 0 }
			map { Scalar::Random::OO::Meta::Class->initialize($_)->get_method_list() }
			$self->linearized_isa;
}
...
sub find_attribute_by_name{
	...
	my $attr;
	foreach my $class($self->linearized_isa){
		my $meta = Scalar::Random::OO::Util::get_metaclass_by_name($class) or next;
...
sub add_attribute {
	...
	if(blessed $_[0]){
		...
		$attr->isa('Scalar::Random::OO::Meta::Attribute')
			|| $self->throw_error("Your attribute must be an instance of Scalar::Random::OO::Meta::Attribute (or a subclass)");
...
sub make_immutable {
	...
	if ($args{inline_constructor}) {
		$self->add_method($args{constructor_name} =>
			Scalar::Random::OO::Util::load_class($self->constructor_class)
				->_generate_constructor($self, \%args));
	}

	if ($args{inline_destructor}) {
		$self->add_method(DESTROY =>
			Scalar::Random::OO::Util::load_class($self->destructor_class)
...
sub _install_modifier {
	...
	if($no_cmm_fast){
		...
		$impl = sub {
			...
			my $method = Scalar::Random::OO::Util::get_code_ref( $into, $name );
			...
			if ( !$method || !Data::Util::subroutine_modifier($method) ) {
				...
				$self->add_method($name => Scalar::Random::OO::Util::get_code_ref($into, $name));
			}
...
sub add_override_method_modifier {
	...
	$self->add_method($name => sub {
		local $Scalar::Random::OO::SUPER_PACKAGE = $package;
		local $Scalar::Random::OO::SUPER_BODY    = $super_body;
		local @Scalar::Random::OO::SUPER_ARGS    = @_;
...
sub add_augment_method_modifier {
	...
	$self->add_method($name => sub{
		local $Scalar::Random::OO::INNER_BODY{$super_package} = $code;
		local $Scalar::Random::OO::INNER_ARGS{$super_package} = [@_];
		$super_body->(@_);
	});
	return;
}
...
package Scalar::Random::OO::Meta::Method;
use Scalar::Random::OO::Util qw(:meta); # enables strict and warnings
use Scalar::Util ();
...
package Scalar::Random::OO::Meta::Method::Accessor;
use Scalar::Random::OO::Util qw(:meta); # enables strict and warnings
...
package Scalar::Random::OO::Meta::Method::Constructor;
use Scalar::Random::OO::Util qw(:meta); # enables strict and warnings
...
sub _generate_constructor {
	...
	my $source = sprintf("#line %d %s\n", __LINE__, __FILE__).<<"...";
		sub \{
			my \$class = shift;
			return \$class->Scalar::Random::OO::Object::new(\@_)
...
sub _generate_BUILDARGS {
	...
	if ( $class->can('BUILDARGS') && $class->can('BUILDARGS') != \&Scalar::Random::OO::Object::BUILDARGS ) {
...
sub _generate_BUILDALL {
	...
	for my $class ($metaclass->linearized_isa) {
		if (Scalar::Random::OO::Util::get_code_ref($class, 'BUILD')) {
...
package Scalar::Random::OO::Meta::Method::Delegation;
use Scalar::Random::OO::Util qw(:meta); # enables strict and warnings
use Scalar::Util;
...
package Scalar::Random::OO::Meta::Method::Destructor;
use Scalar::Random::OO::Util qw(:meta); # enables strict and warnings
...
sub _generate_destructor{
	...
	my $demolishall = '';
	for my $class ($metaclass->linearized_isa) {
		if (Scalar::Random::OO::Util::get_code_ref($class, 'DEMOLISH')) {
			$demolishall .= sprintf "%s::DEMOLISH(\$self, \$Scalar::Random::OO::Util::in_global_destruction);\n",
				$class,
		}
	}
...
package Scalar::Random::OO::Meta::Module;
use Scalar::Random::OO::Util qw/:meta get_code_package get_code_ref not_supported/; # enables strict and warnings
no warnings 'once';
...
if(Scalar::Random::OO::Util::MOUSE_XS){
	# register meta storage for performance
	Scalar::Random::OO::Util::__register_metaclass_storage(\%METAS, 0);

	# ensure thread safety
	*CLONE = sub { Scalar::Random::OO::Util::__register_metaclass_storage(\%METAS, 1) };
}

sub _metaclass_cache { # DEPRECATED
	my($self, $name) = @_;
	Carp::cluck('_metaclass_cache() has been deprecated. Use Scalar::Random::OO::Util::get_metaclass_by_name() instead');
	return $METAS{$name};
}
...
my %foreign = map{ $_ => undef } qw(
	Scalar::Random::OO Scalar::Random::OO::Role Scalar::Random::OO::Util Scalar::Random::OO::Util::TypeConstraints
...
sub get_method{
	...
	if(my $code = $self->get_method_body($method_name)){
		return Scalar::Random::OO::Util::load_class($self->method_metaclass)->wrap(
			body                 => $code,
			name                 => $method_name,
			package              => $self->name,
...
sub _collect_methods { # Scalar::Random::OO specific
	my($meta, @args) = @_;
...
sub create {
	...
	if(exists $options{superclasses}){
		if(Scalar::Random::OO::Util::is_a_metarole($self)){
	...
	if (defined $attributes) {
		if(ref($attributes) eq 'ARRAY'){
			# array of Scalar::Random::OO::Meta::Attribute
	...
	if (defined $roles){
		Scalar::Random::OO::Util::apply_all_roles($package_name, @{$roles});
	}
...
sub DESTROY{
	...
	return if $Scalar::Random::OO::Util::in_global_destruction;
...
package Scalar::Random::OO::Meta::Role;
use Scalar::Random::OO::Util qw(:meta not_supported); # enables strict and warnings
	...
	if($args->{_to} eq 'role'){
		...
		if(@missing){
			$role->throw_error(sprintf "'%s' requires the method%s %s to be implemented by '%s'",
				...
				Scalar::Random::OO::Util::quoted_english_list(@missing),
				$consumer_class_name);
		}
	}
...
sub apply {
	...
	if(Scalar::Random::OO::Util::is_a_metaclass($consumer)){  # Application::ToClass
		$args{_to} = 'class';
	}
	elsif(Scalar::Random::OO::Util::is_a_metarole($consumer)){ # Application::ToRole
		$args{_to} = 'role';
	}
		...
		$consumer = (Scalar::Random::OO::Util::class_of($instance) || 'Scalar::Random::OO::Meta::Class')->create_anon_class(
...
sub combine {
	...
	require 'Scalar/Random/OO/Meta/Role/Composite.pm'; # we don't want to create its namespace

	my $composite = Scalar::Random::OO::Meta::Role::Composite->create_anon_role();
...
package Scalar::Random::OO::Meta::Role::Composite;
use Scalar::Random::OO::Util; # enables strict and warnings
use Scalar::Random::OO::Meta::Role;
our @ISA = qw(Scalar::Random::OO::Meta::Role);
...
sub _apply_methods{
	...
	if(exists $self->{conflicting_methods}){
		...
		if(@conflicting == 1){
			...
			my $roles       = Scalar::Random::OO::Util::quoted_english_list(map{ $_->name } @{ $self->{composed_roles_by_method}{$method_name} });
			$self->throw_error(
			...
			my %seen;
			my $roles = Scalar::Random::OO::Util::quoted_english_list(
				grep{ !$seen{$_}++ } # uniq
				map { $_->name }
			...
			$self->throw_error(
			   sprintf q{Due to method name conflicts in roles %s, the methods %s must be implemented or excluded by '%s'},
				   $roles,
				   Scalar::Random::OO::Util::quoted_english_list(@conflicting),
				   $consumer_class_name
			);
		}
	}
...
package Scalar::Random::OO::Meta::Role::Method;
use Scalar::Random::OO::Util; # enables strict and warnings

use Scalar::Random::OO::Meta::Method;
our @ISA = qw(Scalar::Random::OO::Meta::Method);
...
package Scalar::Random::OO::Object;
use Scalar::Random::OO::Util qw(does dump meta); # enables strict and warnings
...
use Scalar::Random::OO::Util  qw(not_supported);
use Scalar::Random::OO::Meta::Role;
...
sub with     {
	my $meta = Scalar::Random::OO::Meta::Role->initialize(scalar caller);
	Scalar::Random::OO::Util::apply_all_roles($meta->name, @_);
	return;
}
...
sub super {
	return if !defined $Scalar::Random::OO::SUPER_BODY;
	$Scalar::Random::OO::SUPER_BODY->(@Scalar::Random::OO::SUPER_ARGS);
}

sub override {
	# my($name, $code) = @_;
	Scalar::Random::OO::Meta::Role->initialize(scalar caller)->add_override_method_modifier(@_);
	return;
}
...
sub init_meta{
	...
	my $metaclass  = $args{metaclass}  || 'Scalar::Random::OO::Meta::Role';
	...
	# make a role type for each Scalar::Random::OO role
	Scalar::Random::OO::Util::TypeConstraints::role_type($class)
		unless Scalar::Random::OO::Util::TypeConstraints::find_type_constraint($class);
...
package Scalar::Random::OO::Util::MetaRole;
use Scalar::Random::OO::Util; # enables strict and warnings
use Scalar::Util ();
...
sub apply_metaroles {
	...
	my $for = Scalar::Util::blessed($args{for})
		?                                     $args{for}
		: Scalar::Random::OO::Util::get_metaclass_by_name( $args{for} );

	if(!$for){
		Carp::confess("You must pass an initialized class, but '$args{for}' has no metaclass");
	}

	if ( Scalar::Random::OO::Util::is_a_metarole($for) ) {
...
sub _fixup_old_style_args {
	...
	my $top_key;
	if( Scalar::Random::OO::Util::is_a_metaclass($for) ){
...
sub apply_base_class_roles {
	...
	my $meta = Scalar::Random::OO::Util::class_of($for);
...
sub _make_new_class {
	...
	if(!$superclasses){
		...
		my $meta = Scalar::Random::OO::Meta::Class->initialize($existing_class);

		return $existing_class
			if !grep { !ref($_) && !$meta->does_role($_) } @{$roles};
	}

	return Scalar::Random::OO::Meta::Class->create_anon_class(
...
package Scalar::Random::OO;

our $VERSION = '0.06';

Scalar::Random::OO::Exporter->setup_import_methods(also => 'Scalar::Random::OO::TOP');
